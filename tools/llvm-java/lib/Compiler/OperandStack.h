...
namespace llvm {  namespace Java {
  ...
  public:
    explicit OperandStack( Resolver* resolver, unsigned maxDepth)
      : resolver_(resolver),
        currentDepth_(0),
        stack_(maxDepth) { }

    unsigned getDepth()  { return currentDepth_; }
    ...
    /// stack and appends any instructions to implement this to \c
    /// insertAtEnd BasicBlock
    Value* pop( Type* type, BasicBlock* insertAtEnd);
  ...
  private:
    AllocaInst* getOrCreateSlot(SlotMap& slotMap,
                                 Type* type,
                                BasicBlock* bb);
    void copySlots( SlotMap& src, SlotMap& dst, BasicBlock* insertAtEnd);
  };
