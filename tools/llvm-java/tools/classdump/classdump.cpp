...
namespace {
  ...
  public:
    ClassDump( ClassFile* cf, std::ostream& out)
      : CF(cf), Out(out) {
    ...
    std::string getPrettyString( std::string& Desc) {
      unsigned I = 0;
    ...
    std::string getPrettyStringHelper( std::string& Desc, unsigned& I) {
      if (Desc.size() == I)
        return "";
    ...
    void do_lookupswitch(unsigned defTarget,  SwitchCases& sw) { abort(); }
    ...
    void printClassRef(unsigned index) {
       std::string& FQCN = CF->getConstantClass(index)->getName()->str();
...
int main(int argc, char* argv[])
{
  ...
  Java::ClassFile* cf(Java::ClassFile::get(InputClass));
